(require "evaluator.rkt")

; uses picture as background for character sheet
(define sheetpicture (read-bitmap "./DND/ddcharsheet.png"))
(define pic-w (send sheetpicture get-width))
(define pic-h (send sheetpicture get-height))

; main frame for the character sheet
(define charsheet (new frame% [label "Character Sheet"]
                  [width pic-w]
                  [height pic-h]))

  
; canvas on which to draw data
(define cansheet
  (new canvas% [parent charsheet]
       [style '(hscroll vscroll no-focus)]
               [paint-callback
              (λ (canvas dc)
                (send dc draw-bitmap sheetpicture 0 0)
                (send dc set-text-foreground "red")

                ; adds some delta to a value
                (define (add-delta val delta)
                  (+ val delta))
                
                ; prints stats in hash tables onto character sheet
               (let* ((statsx 65) (statsstr 210) (stats_delta_y 95)
                                  (statsdex (add-delta statsstr stats_delta_y))
                                  (statscon (add-delta statsdex stats_delta_y))
                                  (statsint (add-delta statscon stats_delta_y))
                                  (statswis (add-delta statsint stats_delta_y))
                                  (statschar (add-delta statswis stats_delta_y))
                                  (savings_x 150)
                                  (savings_y_delta 18)
                                  (savings_str_y 267)
                                  (savings_dex_y (add-delta savings_str_y savings_y_delta))
                                  (savings_con_y (add-delta savings_dex_y savings_y_delta))
                                  (savings_int_y (add-delta savings_con_y savings_y_delta))
                                  (savings_wis_y (add-delta savings_int_y savings_y_delta))
                                  (savings_cha_y (add-delta savings_wis_y savings_y_delta))
                                  (savings_star_x 133)
                                  (savings_star_str 273)
                                  (savings_star_delta 18)
                                  (savings_star_dex (add-delta savings_star_str savings_star_delta))
                                  (savings_star_con (add-delta savings_star_dex savings_star_delta))
                                  (savings_star_int (add-delta savings_star_con savings_star_delta))
                                  (savings_star_wis (add-delta savings_star_int savings_star_delta))
                                  (savings_star_cha (add-delta savings_star_wis savings_star_delta))
                                  (star "*"))
                 (unless (hash-empty? hash-stats)
                  (for ([(key value) (in-hash hash-stats)])
                    (let ((stats-val (let ((z ((λ (x) (car x)) value))) (number->string z)))
                          (prof-bonus (get-hash-base "character-proficiency-bonus"))
                          (prof (cdr value)))
                      (cond ((equal? key 'stat-strength) (begin (send dc draw-text stats-val statsx statsstr)
                                                                (if prof
                                                                    (begin (send dc draw-text star savings_star_x savings_star_str)
                                                                           (send dc draw-text (number->string (+ prof-bonus (getmod "strength"))) savings_x savings_str_y))
                                                                    (send dc draw-text (number->string (getmod "strength")) savings_x savings_str_y))))
                            ((equal? key 'stat-dexterity) (begin (send dc draw-text stats-val statsx statsdex)
                                                                 (if prof
                                                                     (begin (send dc draw-text star savings_star_x savings_star_dex)
                                                                            (send dc draw-text (number->string (+ prof-bonus (getmod "dexterity"))) savings_x savings_dex_y))
                                                                     (send dc draw-text (number->string (getmod "dexterity")) savings_x savings_dex_y))))
                            ((equal? key 'stat-constitution) (begin (send dc draw-text stats-val statsx statscon)
                                                                    (if prof
                                                                        (begin (send dc draw-text star savings_star_x savings_star_con)
                                                                               (send dc draw-text (number->string (+ prof-bonus (getmod "constitution"))) savings_x savings_con_y))
                                                                        (send dc draw-text (number->string (getmod "constitution")) savings_x savings_con_y))))
                            ((equal? key 'stat-intelligence) (begin (send dc draw-text stats-val statsx statsint)
                                                                    (if prof
                                                                        (begin (send dc draw-text star savings_star_x savings_star_int)
                                                                               (send dc draw-text (number->string (+ prof-bonus (getmod "intelligence"))) savings_x savings_int_y))
                                                                    (send dc draw-text (number->string (getmod "intelligence")) savings_x savings_int_y))))
                            ((equal? key 'stat-wisdom) (begin (send dc draw-text stats-val statsx statswis)
                                                              (if prof
                                                                  (begin (send dc draw-text star savings_star_x savings_star_wis)
                                                                         (send dc draw-text (number->string (+ prof-bonus (getmod "wisdom"))) savings_x savings_wis_y))
                                                                    (send dc draw-text (number->string (getmod "wisdom")) savings_x savings_wis_y))))
                            ((equal? key 'stat-charisma) (begin (send dc draw-text stats-val statsx statschar)
                                                                (if prof
                                                                    (begin (send dc draw-text star savings_star_x savings_star_cha)
                                                                           (send dc draw-text (number->string (+ prof-bonus (getmod "charisma"))) savings_x savings_cha_y))
                                                                    (send dc draw-text (number->string (getmod "charisma")) savings_x savings_cha_y)))))))))

                   ; prints player info
                  (unless (hash-empty? hash-base)
                    (let* ((char_x 90) (char_y 90)
                                  (line1_y 65)
                                  (line2_y 100)
                                  (class_x 350)
                                  (player_x 630)
                                  (align_x 500)
                                  (race_x 350)
                                  (hp_y 257)
                                  (level_x (add-delta class_x 120))
                                  (armor_class_x 325)
                                  (armor_class_y 200)
                                  (speed_x (add-delta armor_class_x 150))) 
                      (for ([(key value) (in-hash hash-base)])
                        (cond ((equal? key 'character-name) (send dc draw-text value char_x char_y))
                              ((equal? key 'character-class) (send dc draw-text value class_x line1_y))
                              ((equal? key 'character-level) (send dc draw-text (if (number? value) (number->string value) value) level_x line1_y))
                              ((equal? key 'player-name) (send dc draw-text value player_x line1_y))
                              ((equal? key 'character-race) (send dc draw-text value race_x line2_y))
                              ((equal? key 'character-alignment) (send dc draw-text value align_x line2_y))
                              ((equal? key 'character-exp) (send dc draw-text (if (number? value) (number->string value) value)  player_x line2_y))
                              ((equal? key 'character-hp) (send dc draw-text (if (number? value) (number->string value) value) (+ class_x 50) hp_y))
                              ((equal? key 'character-armor-class) (send dc draw-text (if (number? value) (number->string value) value) armor_class_x armor_class_y))
                              ((equal? key 'character-speed) (send dc draw-text (if (number? value) (number->string value) value) speed_x armor_class_y))
                              ((equal? key 'character-hit-dice) (send dc draw-text value 325 450))
                              ((equal? key 'character-proficiency-bonus) (send dc draw-text (if (number? value) (number->string value) value) 140 225))
                              (else (send dc draw-text (number->string (getmod "dexterity")) (add-delta armor_class_x 75) armor_class_y))
                              ))))

                ; prints skills
                 (unless (hash-empty? hash-skills)
                   (let* ((skills_x 150) (skills_delta_y 18)
                                  (acrobatics_y 419)
                                  (animal-handling_y (add-delta acrobatics_y skills_delta_y))
                                  (arcana_y (add-delta animal-handling_y skills_delta_y))
                                  (athletics_y (add-delta arcana_y skills_delta_y))
                                  (dception_y (add-delta athletics_y skills_delta_y))
                                  (history_y (add-delta dception_y skills_delta_y))
                                  (insight_y (add-delta history_y skills_delta_y))
                                  (intimidation_y (add-delta insight_y skills_delta_y))
                                  (investigation_y (add-delta intimidation_y skills_delta_y))
                                  (medicine_y (add-delta investigation_y skills_delta_y))
                                  (nature_y (add-delta medicine_y skills_delta_y))
                                  (perception_y (add-delta nature_y skills_delta_y))
                                  (performance_y (add-delta perception_y skills_delta_y))
                                  (persuasion_y (add-delta performance_y skills_delta_y))
                                  (religion_y (add-delta persuasion_y skills_delta_y))
                                  (slight-of-hand_y (add-delta religion_y skills_delta_y))
                                  (stealth_y (add-delta slight-of-hand_y skills_delta_y))
                                  (survival_y (add-delta stealth_y skills_delta_y))
                                  (skills_star_x 133)
                                  (acrobatics_star_y 425)
                                  (animal_handling_star_y (add-delta acrobatics_star_y skills_delta_y))
                                  (arcana_star_y (add-delta animal_handling_star_y skills_delta_y))
                                  (athletics_star_y (add-delta arcana_star_y skills_delta_y))
                                  (dception_star_y (add-delta athletics_star_y skills_delta_y))
                                  (history_star_y (add-delta dception_star_y skills_delta_y))
                                  (insight_star_y (add-delta history_star_y skills_delta_y))
                                  (intimidation_star_y (add-delta insight_star_y skills_delta_y))
                                  (investigation_star_y (add-delta intimidation_star_y skills_delta_y))
                                  (medicine_star_y (add-delta investigation_star_y skills_delta_y))
                                  (nature_star_y (add-delta medicine_star_y skills_delta_y))
                                  (perception_star_y (add-delta nature_star_y skills_delta_y))
                                  (performance_star_y (add-delta perception_star_y skills_delta_y))
                                  (persuasion_star_y (add-delta performance_star_y skills_delta_y))
                                  (religion_star_y (add-delta persuasion_star_y skills_delta_y))
                                  (slight_of_hand_star_y (add-delta religion_star_y skills_delta_y))
                                  (stealth_star_y (add-delta slight_of_hand_star_y skills_delta_y))
                                  (survival_star_y (add-delta stealth_star_y skills_delta_y))
                                  (star "*")
                                  )
                   (for ([(key value) (in-hash hash-skills)])
                     (let ((val-num (let ((z ((λ (x) (get-skill-mod x)) key))) (number->string z)))
                           (prof (cdr value)))
                       (cond ((equal? key 'acrobatics) (begin (send dc draw-text val-num skills_x acrobatics_y)
                                                              (when prof (send dc draw-text star skills_star_x 425))))
                             ((equal? key (string->symbol "animal handling")) (begin (send dc draw-text val-num skills_x animal-handling_y)
                                                                                     (when prof (send dc draw-text star skills_star_x animal_handling_star_y))))
                             ((equal? key 'arcana) (begin (send dc draw-text val-num skills_x arcana_y)
                                                          (when prof (send dc draw-text star skills_star_x 461))))
                             ((equal? key 'athletics) (begin (send dc draw-text val-num skills_x athletics_y)
                                                             (when prof (send dc draw-text star skills_star_x athletics_star_y))))
                             ((equal? key 'dception) (begin (send dc draw-text val-num skills_x dception_y)
                                                            (when prof (send dc draw-text star skills_star_x dception_star_y))))
                             ((equal? key 'history) (begin (send dc draw-text val-num skills_x history_y)
                                                           (when prof (send dc draw-text star skills_star_x history_star_y))))
                             ((equal? key 'insight) (begin (send dc draw-text val-num skills_x insight_y)
                                                           (when prof (send dc draw-text star skills_star_x insight_star_y))))
                             ((equal? key 'intimidation) (begin (send dc draw-text val-num skills_x intimidation_y)
                                                                (when prof (send dc draw-text star skills_star_x intimidation_star_y))))
                             ((equal? key 'investigation) (begin (send dc draw-text val-num skills_x investigation_y)
                                                                 (when prof (send dc draw-text star skills_star_x investigation_star_y))))
                             ((equal? key 'medicine) (begin (send dc draw-text val-num skills_x medicine_y)
                                                            (when prof (send dc draw-text star skills_star_x medicine_star_y))))
                             ((equal? key 'nature) (begin (send dc draw-text val-num skills_x nature_y)
                                                          (when prof (send dc draw-text star skills_star_x nature_star_y))))
                             ((equal? key 'perception) (begin (send dc draw-text val-num skills_x perception_y)
                                                              (when prof (send dc draw-text star skills_star_x perception_star_y))))
                             ((equal? key 'performance) (begin (send dc draw-text val-num skills_x performance_y)
                                                               (when prof (send dc draw-text star skills_star_x performance_star_y))))
                             ((equal? key 'persuasion) (begin (send dc draw-text val-num skills_x persuasion_y)
                                                              (when prof (send dc draw-text star skills_star_x persuasion_star_y))))
                             ((equal? key 'religion) (begin (send dc draw-text val-num skills_x religion_y)
                                                            (when prof (send dc draw-text star skills_star_x religion_star_y))))
                             ((equal? key (string->symbol "slight of hand")) (begin (send dc draw-text val-num skills_x slight-of-hand_y)
                                                                                    (when prof (send dc draw-text star skills_star_x slight_of_hand_star_y))))
                             ((equal? key 'stealth) (begin (send dc draw-text val-num skills_x stealth_y)
                                                           (when prof (send dc draw-text star skills_star_x stealth_star_y))))
                             ((equal? key 'survival) (begin (send dc draw-text val-num skills_x survival_y)
                                                            (when prof (send dc draw-text star skills_star_x survival_star_y))))
                             )))))

                ; helper function that prints out list that contains elements of cons items
                (define (print-list lst x y delta)
                             (unless (equal? lst '())
                               (let ((item (cadar lst)) (quantity (cddar lst)))
                                     (begin (send dc draw-text
                                                  (if (number? quantity) (string-append (if (number? quantity) (number->string quantity) quantity) " " item)
                                                      (string-append quantity " " item)) x y)
                                            (print-list (cdr lst) x (add-delta y delta) delta)))))
                
                ; prints weapons list
                (unless (hash-empty? hash-weapons)
                  (let ((weapons-list (hash->list hash-weapons)))
                    (print-list weapons-list 300 590 15)))

                             
                ; prints inventory of equipment
               (unless (hash-empty? hash-inventory)
                 (let ((inventory-list (hash->list hash-inventory)))
                     (print-list inventory-list 355 780 15)))

                ; prints proficiencies
                (unless (hash-empty? hash-proficiencies-list)
                  (define proficiencies (hash->list hash-proficiencies-list))
                  (define (print-proficiencies lst x-coord y-coord delta)
                    (unless (equal? lst '())
                      (let ((prof (cadar lst)))
                        (begin (send dc draw-text prof x-coord y-coord)
                               (print-proficiencies (cdr lst) x-coord (add-delta y-coord delta) delta)))))
                  (print-proficiencies proficiencies 45 823 15))

                
               ; prints  notes
                (unless (hash-empty? hash-notes)
                  (letrec ((notes (map string-split (map cdr (hash->list hash-notes))))
                           (line-length 25)
                           (x-coord 545)
                           (y-coord 502)
                           (delt 15)
                           (print-string (λ (lst str x y delta)
                                 (cond ((equal? lst '()) (begin (send dc draw-text str x y) (send dc draw-text "" x (add-delta y delta)) (add-delta y (* 2 delta))))
                                       (else (let ((z (string-append str " " (car lst))))
                                               (if (> (string-length z) line-length) (begin (send dc draw-text str x y) (print-string lst "" x (+ y delta) delta))
                                                   (print-string (cdr lst) z x y delta)))))))
                           (print-notes (λ (lst str x y delta)
                                 (unless (equal? lst '())
                                   (print-notes (cdr lst) str x (print-string (car lst) str x y delta) delta)))))
                    (print-notes notes "" x-coord y-coord delt)))

                ; calculates perception bonus
                (define (calc-perception-bonus)
                  (+ (getmod "wisdom") (if (cdr (hash-ref hash-skills 'perception)) (get-hash-base "character-proficiency-bonus") 0)))

                ; calculates passive wisdom
                (define (calc-passive-wisdom)
                  (+ 10 (calc-perception-bonus)))

                ;prints passive wisdom
                (send dc draw-text (number->string (calc-passive-wisdom))  50 785)
                )]))

 
; boolean parameter determines whether to draw character sheet or not
(define (genCS showsheet)
  (cond ((eq? showsheet #t) (begin (send cansheet on-paint)
                                   (send cansheet init-auto-scrollbars pic-w pic-h 0 0)
                                   (send charsheet show #t)))
        (else (send charsheet on-exit))))
       
  